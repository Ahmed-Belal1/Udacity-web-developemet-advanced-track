...
def venues():
  ...
  for d in res:
    flag =False
    for x in data:
      if(d.city == x['city'] and d.state== x['state']):
        flag=True
        break
      ...
      for x in data:
        if(d.city == x['city'] and d.state== x['state']):
          x['venues'].append({
            "id": d.id,
            "name": d.name,  
            "num_upcoming_shows": len(db.session.query(Show).filter(Show.venue_id==d.id).filter(Show.start_date > datetime.now()).all())
          })   
    else:
      data.append({
        "city": d.city,
...
def search_venues():
  ...
  if search_word=="":
    ...
    for d in res :
      if search_word.lower() in d.name.lower() :
        count=count+1
        data.append({
          "id": d.id,
          "name": d.name,  
...
def show_venue(venue_id):
  ...
  artist_shows= db.session.query(Artist, Show).join(Show).join(Venue).\
    filter(
  ...
  for a,s in artist_shows: 
    name=a.name
    image=a.image_link
    if(s.start_date <=  datetime.now() ):
      pastshowscount= pastshowscount+1
      pastshows.append({
        "artist_id":a.id ,
        "artist_name": name,
        "artist_image_link": image,
        "start_time": format_datetime(str(s.start_date))
      })
    else:
      upcomingshowscount= upcomingshowscount+1
  ...
  data={
    "id": venue_id,
    "name": res.name,
    "genres": res.genres if not res.genres ==None else  [],
    "address": res.address,
    "city": res.city,
    "state": res.state,
    "phone": res.phone,
...
def search_artists():
  ...
  for a in res:
    if(search_word.lower() in a.name.lower()):
      count=count+1
      num =len(db.session.query(Show).filter(Show.artist_id==a.id).filter(Show.start_date > datetime.now()).all())
      data.append({
        "id":a.id,
        "name":a.name,
...
def show_artist(artist_id):
  ...
  venue_shows= db.session.query(Venue, Show).join(Show).join(Artist).\
    filter(
  ...
  upcomingshows=[]
  upcomingshowscount=0
  for a,s in venue_shows: 
    a=Venue.query.get(s.venue_id)
    name=a.name
    image=a.image_link
    if(s.start_date <=  datetime.now() ):
      pastshowscount= pastshowscount+1
